...
public class CommandEcosign extends CommandExec {
	...
	public CommandEcosign(final IPlayerConfEntry entry, final ILand land, final Action action,
			final SignType signType) throws FactoidCommandException {
	...
	public void commandExecute() throws FactoidCommandException {
		if (action == Action.RIGHT_CLICK_BLOCK) {
			if (signType == SignType.SALE) {
				...
				// Rent and unrent
				if (land.isRented() &&
					(land.getTenant().hasAccess(player) || land.getOwner().hasAccess(player) || playerConf.isAdminMod())
				) {
					// Unrent
					((Land) land).unSetRented();
					try {
						new EcoSign(land, land.getRentSignLoc())
							.createSignForRent(
								land.getRentPrice(),
								land.getRentRenew(),
