...
public class CommandSelectWorldedit {
	 ...
	 */
	public CommandSelectWorldedit(final Player player, final PlayerConfEntry entry) throws FactoidCommandException{
	...
	protected void MakeSelect() throws FactoidCommandException {
		...
		final LocalSession session = ((WorldEditPlugin) Factoid.getThisPlugin().iDependPlugin().getWorldEdit()).getSession(player);
		...
		try {
			...
			final AreaSelection select = new AreaSelection(player, new CuboidArea(player.getWorld().getName(),
