...
public class CommandSelect extends CommandExec {
	 ...
	 */
	public CommandSelect(final CommandEntities entity) throws FactoidCommandException {
		super(entity);
	 ...
	 * @throws FactoidCommandException the factoid command exception
	 */
	public CommandSelect(final Player player, final ArgList argList, final Location location) throws FactoidCommandException {
		super(null);
		this.player = player;
		...
		if (playerConf.getSelection().getCuboidArea() == null) {
			...
			if (!argList.isLast()) {
				...
				if (curArg.equalsIgnoreCase("worldedit")) {
					...
					if (!playerConf.isAdminMod() &&
						!landtest.isOwner(player) &&
						!landtest.isTenant(player) &&
				...
				final ActiveAreaSelection select = new ActiveAreaSelection(player);
