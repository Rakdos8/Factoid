...
public class CommandApprove extends CommandExec {
	 ...
	 */
	public CommandApprove(final CommandEntities entity) throws FactoidCommandException {
	...
	public void commandExecute() throws FactoidCommandException {
		...
		final ApproveList approveList = Factoid.getThisPlugin().iLands().getApproveList();
		final boolean isApprover = entity.sender.hasPermission("factoid.collisionapprove");
		...
		if (curArg.equalsIgnoreCase("clear")) {
			...
			final String param = entity.argList.getNext();

			if (param == null) {
				throw new FactoidCommandException("Approve", entity.sender, "COLLISION.SHOW.PARAMNULL");
			}

			final Approve approve = approveList.getApprove(param);
			...
			final ILand land = Factoid.getThisPlugin().iLands().getLand(param);
			final Collisions.LandAction action = approve.getAction();
			final int removeId = approve.getRemovedAreaId();
			final ICuboidArea newArea = approve.getNewArea();
			final ILand parent = approve.getParent();
			final Double price = approve.getPrice();
			final IPlayerContainer owner = approve.getOwner();
