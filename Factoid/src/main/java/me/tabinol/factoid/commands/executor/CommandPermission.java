...
public class CommandPermission extends CommandThreadExec {
	 ...
	 */
	public CommandPermission(final CommandEntities entity) throws FactoidCommandException {
	...
	private void importDisplayPermsFrom(final IDummyLand land, final boolean onlyInherit) {
	...
	public void commandThreadExecute(final PlayerCacheEntry[] playerCacheEntry)
		...
		if (fonction.equalsIgnoreCase("set")) {
			...
			final IPermissionType pt = entity.argList.getPermissionTypeFromArg(entity.playerConf.isAdminMod(), land.isOwner(entity.player));
